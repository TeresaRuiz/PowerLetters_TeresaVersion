...
SAVE_FORM.addEventListener('submit', async (event) => {
    ...
    (ID_autor.value) ? action = 'updateRow' : action = 'createRow';
    // Constante tipo objeto con los datos del formulario.
    const FORM = new FormData(SAVE_FORM);
    // Petición para guardar los datos del formulario.
    const DATA = await fetchData(AUTORES_API, action, FORM);
        ...
        DATA.dataset.forEach(row => {
            ...
            <tr>
                ...
                <td class="action-icons">
                    <a onclick="openUpdate(${row.id_autor})">
                        <i class="ri-edit-line"></i>
                    </a>
                    <a onclick="openDelete(${row.id_autor})">
...
const openDelete = async (id) => {
    ...
    if (RESPONSE) {
        ...
        const DATA = await fetchData(AUTORES_API, 'deleteRow', FORM);
        // Se comprueba si la respuesta es satisfactoria, de lo contrario se muestra un mensaje con la excepción.
        if (DATA.status) {
            // Se muestra un mensaje de éxito.
